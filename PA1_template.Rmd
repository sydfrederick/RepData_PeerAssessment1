...
Our mean is `r mean`. It's no surprise that the median is `r median` with so many observations taken with zero steps.
...
It seems our median has remained at `r median `, however our mean has changed, but only slightly, from `r mean` to `r new_mean`.
